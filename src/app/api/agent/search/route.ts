...
async function searchSanciones(query: string) {
  // 4n4q-k399 — Sanciones contractuales a contratistas
  const q = query.toUpperCase();
  const url = `https://www.datos.gov.co/resource/4n4q-k399.json?$limit=20&$where=upper(nombre_contratista) like '%25${q}%25' OR upper(nombre_entidad) like '%25${q}%25'`;
  try {
    const res = await fetch(url, { signal: AbortSignal.timeout(10000), cache: "no-store" });
    const data = await res.json();
    return Array.isArray(data) ? data : [];
...
function calcularRiesgo(contratos: any[], contraloria: any[], sanciones: any[]) {
  ...
  const montoSanciones = sanciones.reduce((acc, c) => acc + (parseFloat(c.valor_sancion) || 0), 0);

  if (total > 10 && entidades < 3) {
    score += 25;
    alertas.push(`Concentración: ${total} contratos en solo ${entidades} entidades`);
  }
  if (total > 0 && directos / total > 0.6) {
    score += 20;
    alertas.push(`${directos} de ${total} contratos por contratación directa (${Math.round(directos / total * 100)}%)`);
  }
  if (contraloria.length > 0) {
    score += 35;
    alertas.push(`CRÍTICO: ${contraloria.length} fallo(s) de responsabilidad fiscal — $${(montoContraloria / 1e9).toFixed(1)}B COP`);
  }
  if (sanciones.length > 0) {
    score += 20;
    alertas.push(`${sanciones.length} sanción(es) contractual(es) — multas por $${(montoSanciones / 1e6).toFixed(0)}M COP`);
  }
  if (valorTotal > 1_000_000_000) {
    score += 10;
    alertas.push(`Valor total contratado: $${(valorTotal / 1e9).toFixed(1)}B COP`);
  }
...
export async function GET(req: Request) {
  ...
  const sanciones   = r4.status === "fulfilled" ? r4.value : [];
  const contratos   = [...secopII, ...secopI];
  const riesgo      = calcularRiesgo(contratos, contraloria, sanciones);

  console.log(`>>> SECOP II:${secopII.length} SECOP I:${secopI.length} CGR:${contraloria.length} Sanciones:${sanciones.length}`);

  return NextResponse.json({
    query,
    fuentes: {
      secopII: secopII.length,
      secopI: secopI.length,
      contraloria: contraloria.length,
      sancionesContractuales: sanciones.length,
    },
    riesgo,
